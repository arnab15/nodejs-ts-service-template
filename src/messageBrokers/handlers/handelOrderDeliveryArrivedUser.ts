...
import { IHandelDeliveryArrivedUser } from '../Interfaces/MessageType';
...
export const handelDeliveryArrivedUser = async (
  deliveryArrivedUserPayload: IHandelDeliveryArrivedUser,
): Promise<void> => {
  const communicationTemplate = await communicationTemplateRepository.getCommunicationTemplate(
    deliveryArrivedUserPayload.type,
    NotificationChannel.PUSH,
  );
  if (!communicationTemplate) {
    logger.error(`no communication template found for type ${deliveryArrivedUserPayload.type}`);
    return;
  }

  const recipientUser = await recipientRepository.getRecipientDetailsByEntityId(
    deliveryArrivedUserPayload.data.userId,
    EntityType.USER,
  );
  const recipientDeliveryAgent = await recipientRepository.getRecipientDetailsByEntityId(
    deliveryArrivedUserPayload.data.deliveryPartnerId,
    EntityType.DELIVERY_PARTNER,
  );
  if (!recipientUser) {
    logger.error(`no recipient found for user id ${deliveryArrivedUserPayload.data.userId}`);
    return;
  }
  if (!recipientDeliveryAgent) {
    logger.error(
      `no recipient found for delivery agent id ${deliveryArrivedUserPayload.data.deliveryPartnerId}`,
    );
    return;
  }
  const tokens = await recipientTokenRepository.getTokensByRecipient(`${recipientUser._id}`);
  if (tokens.length === 0) {
    logger.error(`no tokens found for user id ${deliveryArrivedUserPayload.data.userId}`);
    return;
  }
  const renderedTemplate = await renderTemplate({
    topic: deliveryArrivedUserPayload.type,
    channel: NotificationChannel.PUSH,
    lang: 'en',
    variables: {
      orderId: deliveryArrivedUserPayload.data.orderId,
