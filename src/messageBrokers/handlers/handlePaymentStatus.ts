...
import { IHandelPaymentFailed } from '../Interfaces/MessageType';
...
export const handelPaymentStatusFailed = async (
  paymentPayload: IHandelPaymentFailed,
): Promise<void> => {
  ...
  if (!communicationTemplate) {
    logger.error(`no communication template found for type ${paymentPayload.type}`);
    return;
  }

  const recipient = await recipientRepository.getRecipientDetailsByEntityId(
    paymentPayload.data.userId,
    EntityType.USER,
  );
  if (!recipient) {
    logger.error(`no recipient found for user id ${paymentPayload.data.userId}`);
    return;
  }
  const tokens = await recipientTokenRepository.getTokensByRecipient(`${recipient._id}`);
  if (tokens.length === 0) {
    logger.error(`no tokens found for user id ${paymentPayload.data.userId}`);
    return;
  }
  const renderedTemplate = await renderTemplate({
    topic: paymentPayload.type,
    channel: NotificationChannel.PUSH,
    lang: 'en',
    variables: {
      orderId: paymentPayload.data.orderId,
